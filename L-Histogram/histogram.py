...
# histogram bir görüntüdeki ton dağılımını gösterir
...
# aldığımız resmin histogramını çıkarıyoruz 
img_hist = cv2.calcHist([img], channels = [0], mask = None, histSize = [256], ranges = [0,256])
print(img_hist.shape)
plt.figure(), plt.plot(img_hist)

# histogramı renkleri belli olacak şekilde çizdiriyoruz 
color = ("b", "g", "r")
plt.figure()
...
# resim büyük olduğu için resme maske uyguluyoruz 
mask = np.zeros(golden_gate.shape[:2], np.uint8)
plt.figure(), plt.imshow(mask, cmap = "gray")  

# maskede bir delik oluşturuyoruz (siyak maskede beyaz bir bölge oluşturma)
mask[1500:2000, 1000:2000] = 255
plt.figure(), plt.imshow(mask, cmap = "gray") 

# oluşan beyaz bölgeyi orjinal resim haline getiriyoruz 
masked_img_vis = cv2.bitwise_and(golden_gate_vis, golden_gate_vis, mask = mask)
plt.figure(), plt.imshow(masked_img_vis, cmap = "gray") 

# maskeden aldığımız kısmın histogramını oluşturuyouz 
...
# karşıtlığı arttırmadan önceki histogram değerleri 
img_hist = cv2.calcHist([img], channels = [0], mask = None, histSize = [256], ranges = [0,256])
plt.figure(), plt.plot(img_hist)

# karşıtlığı artırıyoruz 
eq_hist = cv2.equalizeHist(img)
plt.figure(), plt.imshow(eq_hist, cmap = "gray") 

# karşıtlığı arttırdığımız resmin histogramı
